...
function showError(msg, severity) {
  ...
  // Toggle banner palette: amber for transient/warning conditions
  // (e.g. cluster restarting), red for hard errors. Class names
  // are listed literally so Tailwind's CDN JIT picks them up.
  var amberCls = ['bg-amber-500/10', 'border-amber-500/20', 'text-amber-300'];
  var redCls   = ['bg-red-500/10', 'border-red-500/20', 'text-red-300'];
  var add = (severity === 'warning') ? amberCls : redCls;
  var rem = (severity === 'warning') ? redCls : amberCls;
  for (var i = 0; i < rem.length; i++) errorBanner.classList.remove(rem[i]);
  for (var i = 0; i < add.length; i++) errorBanner.classList.add(add[i]);
...
function receiveReplica(prompt) {
  ...
  ws.onmessage = function(event) {
    ...
    if (!response.ok) {
      ...
      if (response.code === 'cluster_unavailable' || errMsg.indexOf('MissingBlocksError') >= 0) {
        // Cluster is down or no node is serving the needed blocks.
        // Friendly amber message — usually transient, comes back in a few minutes.
        showError('Our AI cluster is currently restarting. This usually takes a few minutes. Please try again shortly.', 'warning');
      } else if (errMsg.indexOf('out of capacity') >= 0 || errMsg.indexOf('Too many concurrent') >= 0) {
...
var SOURCES_API = '/api/curated-sources';

function openSources() {
  document.getElementById('sources-modal').classList.remove('hidden');
  document.getElementById('src-loading').classList.remove('hidden');
  document.getElementById('src-content').classList.add('hidden');
  document.getElementById('src-empty').classList.add('hidden');
  document.getElementById('src-error').classList.add('hidden');
  fetchSources();
}
...
function fetchSources() {
  ...
  fetch(SOURCES_API, { headers: { 'Accept': 'application/json' } })
    .then(function(r) {
    ...
    .then(function(data) {
      ...
      var count = (data && data.count) || sources.length;
      document.getElementById('src-count-badge').textContent = count + (count === 1 ? ' source' : ' sources');
      if (sources.length === 0) {
    ...
    .catch(function(err) {
      document.getElementById('src-loading').classList.add('hidden');
      document.getElementById('src-error').classList.remove('hidden');
      document.getElementById('src-error-text').textContent = 'Failed to load sources: ' + err.message;
    });
}
